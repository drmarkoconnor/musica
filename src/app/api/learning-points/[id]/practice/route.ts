...
import {
  createPracticeFromLearningPoint,
  isLearningPointId,
  LearningPointError,
...
export async function POST(
  ...
  try {
    const input = parseLearningPointPractice(body);
    ...
    return NextResponse.json(await createPracticeFromLearningPoint(id, input));
  } catch (error) {
    if (error instanceof LearningPointError) {
      return NextResponse.json({ error: error.message }, { status: error.status });
    }
    console.error("Learning point practice creation failed", error);
    return NextResponse.json({ error: "The practice item could not be saved." }, { status: 500 });
  }
}
